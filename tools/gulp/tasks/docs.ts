...
// `layout-docs-example` attribute which can be used to locate the divs and initialize the example
// viewer.
const EXAMPLE_PATTERN = /<!--\W*example\(([^)]+)\)\W*-->/g;

// Markdown files can contain links to other markdown files.
// Most of those links don't work in the Layout docs, because the paths are invalid in the
...
task('docs', sequenceTask(
  [
    ...
    // 'build-highlighted-examples', TODO: add examples
...
/** Generates html files from the markdown overviews and guides for Layout. */
task('markdown-docs-layout', () => {
...
task('build-highlighted-examples', () => {
  ...
  return src('src/layout-examples/**/*.+(html|css|ts)')
    .pipe(flatten())
...
task('copy-stackblitz-examples', () => {
  src(path.join(packagesDir, 'layout-examples', '**/*'))
...
/** Fixes paths in the markdown files to work in the layout-docs-io. */
function fixMarkdownDocLinks(link: string, filePath: string): string {
  ...
  // guides can be loaded in the Layout docs.
