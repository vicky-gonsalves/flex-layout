...
export class ComponentGroup {
  ...
  /** Additional constants that belong to the component group. */
  additionalConsts: CategorizedClassDoc[] = [];
...
export class ComponentGrouper implements Processor {
  ...
  $process(docs: DocCollection) {
    ...
    docs.forEach(doc => {
      ...
      let moduleImportPath = `@angular/${packageName}`;

      // Everything is imported top-level except the separate
      // entry-points
      if (documentInfo.entryPointName === 'server') {
        moduleImportPath += `/${documentInfo.entryPointName}`;
      }
      ...
      group.packageDisplayName = packageName;
      ...
      if (doc.isDirective) {
        ...
        group.additionalConsts.push(doc);
      }
    });
...
function getDocumentPackageInfo(doc: Document) {
  ...
  // For Layout, the directives are nested in subdirectories that can
  // all be imported from the top-level
  const pathSegments = path.relative(basePath, filePath).split(path.sep);
  let groupName = pathSegments[3];

  return {
    name: groupName,
    packageName: 'flex-layout',
